<template>
  <div id="stats" class="stats">
    <ul class="stats__items">
      <li v-tippy title="Number of trades">
        <Measurement
          v-if="statsGraphs"
          ref="tradesMeasurement"
          :timeframe="statsGraphsTimeframe"
          :length="statsGraphsLength"
        />
        <div class="stats__label">TRADES</div>
        <div class="stats__value">{{ $root.formatAmount(totalOrders) }}</div>
      </li>
      <li v-tippy title="Average order">
        <Measurement
          v-if="statsGraphs"
          ref="avgMeasurement"
          :timeframe="statsGraphsTimeframe"
      ...
      <li v-tippy title="Volume delta">
        <Measurement
          v-if="statsGraphs"
          ref="volDeltaMeasurement"
          :timeframe="statsGraphsTimeframe"
          :length="statsGraphsLength"
        />
      ...
      <li v-tippy title="Order delta">
        <Measurement
          v-if="statsGraphs"
          ref="countDeltaMeasurement"
          :timeframe="statsGraphsTimeframe"
          :length="statsGraphsLength"
        />
        <div class="stats__label">TRADES &Delta;</div>
        <div class="stats__value">{{ $root.formatAmount(countDelta, 1) }}</div>
      </li>
    </ul>
  </div>
</template>
  ...
  methods: {
    onTrades(trades, upVolume, downVolume) {
      for (let i = 0; i < trades.length; i++) {
        ...
        this["vol" + side].count +=
          trades[i][3] * (this.preferQuoteCurrencySize ? trades[i][2] : 1)
      }
    },
    ...
    rebuildStats() {
      ...
      socket
        ...
        .forEach(trade => {
          ...
          this["count" + side].count++
          this["vol" + side].count +=
            trade[3] * (this.preferQuoteCurrencySize ? trade[2] : 1)
        })
    ...
    updateStats(timestamp = null) {
      ...
      if (this.statsGraphs) {
        ...
        this.$refs.avgMeasurement.append(
          this.totalOrders ? this.totalVolume / this.totalOrders : 0
        )
